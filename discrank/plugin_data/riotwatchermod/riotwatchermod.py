...
class RiotWatcher:
    ...
    def base_request(self, url, region, static=False, **kwargs):
        ...
        for k in kwargs:
            if isinstance(kwargs[k], bool):
                args[k] = 'true' if kwargs[k] else 'false'
            elif kwargs[k] is not None:
                args[k] = kwargs[k]
        r = requests.get(
            'https://{proxy}.api.riotgames.com/api/lol/{static}{region}/{url}'.format(
    ...
    def _observer_mode_request(self, url, proxy=None, **kwargs):
        ...
        for k in kwargs:
            if kwargs[k] is not None:
                args[k] = kwargs[k]
        r = requests.get(
            'https://{proxy}.api.riotgames.com/observer-mode/rest/{url}'.format(
                proxy=proxy,
                url=url
            ),
            params=args
    ...
    def get_match(self, match_id, region=None, include_timeline=False):
        return self._match_request(
            ...
            # includeTimeline='true' if include_timeline else 'false'
        )
